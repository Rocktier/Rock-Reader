...
| 方案 | 体积成本 | 判断 |
|---|---|---|
| **A. 列系统已装字体**（`getSystemFontList()`） | **0** | ✅ 已实现：用户若在系统里装了喜欢的字体（主题字体），阅读器直接列出来可选 |
| B. 内置 1 款**子集**字体（GB2312 常用字 + 标点 + 拉丁） | **+2~4 MB**（需 fonttools 子集化） | ⏸ 未采用：仍会吃掉 5MB 预算的大半 |
| C. 内置 3 款全字库 | **+33~55 MB** | ❌ 撞碎铁律 2 |
| **D. 按需下载字体包** | **0**（+用户主动联网一次） | ✅ **党哥 2026-09-18 定：走这条路** —— 不增加安装包体积 |
...
- **测量与渲染必须同源**（关键）：ArkUI 的 `registerFont` 只作用于渲染端；
  ...
  —— 不挂的话测量用回退字体断行、渲染用真字体 → 行数不同 → **页面溢出**
- **失败静默**：无网络/超时/校验不过都只写在字体区状态里，不弹窗、不拦路
- **联网登记**：已在家族 `findings.md` §8 登记为"用户主动下载字体包"（不上传任何数据）
